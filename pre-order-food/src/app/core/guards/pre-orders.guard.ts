...
export class PreOrdersGuard implements CanActivate {
  ...
  checkStore(id): Observable<boolean> {
    return this.store.select(fromCoreStore.getPreOrdersLoaded).pipe(
      tap(loaded => {
        this.store.dispatch(new fromCoreStore.LoadPreOrders(id));
      }),
      take(1)
    );
  }
